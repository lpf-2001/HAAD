...
base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"../")  # D:/XRT/HNU/Dataset/网络数据包流量/WF-Datasets/
...
def load_sirinam_dataset(input_size=200, num_classes=95, formatting=True):
    """
...
def load_rimmer_dataset(input_size=200, num_classes=100, formatting=True):
    """
    Load Rimmer's (NDSS'18) dataset.
    """
    # Point to the directory storing data
    dataset_dir = base_path+'utils/dataset/Rimmer/'
...
def load_awf_dataset(input_size=200, num_classes=103, formatting=True, file_name='awf2_remain_small200and100.npz'
...
def load_rimmer_concept_drift_data(time_gap, names, input_size=200, formatting=True):
    """
